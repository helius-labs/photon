...
async fn main() -> anyhow::Result<()> {
    let snapshot_dir = std::env::args().nth(1).expect("Please provide snapshot directory");
    ...
    let target_tree_pubkey = target_tree.as_ref().map(|s| s.parse::<solana_pubkey::Pubkey>().unwrap());
    
    let blocks: Vec<_> = block_stream.collect::<Vec<_>>().await.into_iter().flatten().collect();
                ...
                Ok(state_update) => {
                    ...
                    if has_compression {
                        ...
                        for nullification in &state_update.leaf_nullifications {
                            *tree_transactions.entry(nullification.tree.to_string()).or_insert(0) += 1;
                        ...
                        for address in &state_update.batch_new_addresses {
                            *tree_transactions.entry(address.tree.0.to_string()).or_insert(0) += 1;
    ...
    println!("Compression transactions: {} ({:.2}%)", 
    ...
    if let Some(tree) = target_tree {
        ...
        println!("Blocks containing target tree: {} ({:.2}%)", 
