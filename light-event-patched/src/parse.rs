...
fn create_nullifier_queue_indices(
    ...
    // Fix: size the indices vec to match input_merkle_tree_pubkeys (which i indexes into),
    // not the filtered batch_input_accounts count. The caller zips with batch_input_accounts
    // so extra entries are harmlessly ignored.
    let mut nullifier_queue_indices = vec![u64::MAX; input_merkle_tree_pubkeys.len()];
