...
pub async fn get_queue_elements(
    ...
    let merkle_tree_pubkey = Pubkey::try_from(merkle_tree_pubkey_vec.clone())
        .map_err(|e| PhotonApiError::UnexpectedError(format!("Invalid tree pubkey: {:?}", e)))?;
    let merkle_tree_pubkey_str = bytes_to_sql_format(conn.get_database_backend(), merkle_tree_pubkey_vec);

    info!(
        "Getting queue elements for merkle tree {}",
    ...
    let queue_type_filter = match queue_type {
        ...
        _ => Err(PhotonApiError::ValidationError(format!("Invalid queue type: {:?}", queue_type))),
    }?;
    ...
    let queue_element_map: HashMap<u64, &QueueElement> = queue_elements.iter()
        .map(|e| (e.leaf_index as u64, e))
        .collect();

    let indices: Vec<u64> = queue_elements
        .iter()
        .map(|e| e.leaf_index as u64)
        .collect();
    ...
    let result: Vec<MerkleProofWithContextV2> = proofs.into_iter()
        .filter_map(|proof| {
            queue_element_map.get(&(proof.leaf_index as u64)).map(|queue_element| {
                let tx_hash = queue_element.tx_hash.as_ref().map(|tx_hash| Hash::try_from(tx_hash.clone()).unwrap());
