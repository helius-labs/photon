...
use super::types::{
    GetAccountInterfaceRequest, GetAccountInterfacesRequest, GetAccountInterfacesResponse,
    MAX_BATCH_SIZE,
};

/// Batch fetch generic account data from either on-chain or compressed sources.
pub async fn get_account_interfaces(
    ...
    let futures: Vec<_> = request
        ...
        .map(|address| {
            get_account_interface(
                conn,
                rpc_client,
                GetAccountInterfaceRequest { address: *address },
            )
        })
        .collect();
    ...
    let value = results
        ...
        .map(|(i, r)| match r {
            ...
            Err(e) => {
                log::warn!(
                    "Failed to fetch account interface for {:?} (index {}): {:?}",
    ...
    Ok(GetAccountInterfacesResponse { context, value })
}
