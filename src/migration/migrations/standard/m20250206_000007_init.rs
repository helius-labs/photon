...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        if manager.get_database_backend() == DatabaseBackend::Sqlite {
            // SQLite modifications need to be done by recreating tables
            execute_sql(
                manager,
                r#"
                ...
                SELECT
                    hash, data, data_hash, address, owner, tree, leaf_index, seq,
                    slot_created, spent, prev_spent, lamports, discriminator,
                    TRUE as in_queue, NULL as nullifier, NULL as tx_hash
                FROM accounts;
            ...
            // For PostgreSQL, we can use the WHERE clause in the index
            execute_sql(
                manager,
