...
async fn test_fill_v1_address_tree_gaps() -> Result<()> {
    ...
    println!("🔍 Found {} gaps to fill across all StateUpdate fields:", gaps.len());
    ...
    println!("🎯 Processing all {} gaps across all StateUpdate fields", gaps.len());
    ...
    if !missing_blocks.is_empty() {
        ...
        println!("✅ Updated snapshot with {} signature-based blocks", missing_blocks.len());
    }
    ...
    if remaining_gaps.is_empty() {
        ...
        println!("⚠️  Still have {} gaps - triggering slot-range fallback", remaining_gaps.len());
        ...
        let (fallback_blocks, fallback_updates) = validate_and_fallback_gap_filling(&client, &remaining_gaps, &updated_existing_slots).await?;
        ...
        if !fallback_blocks.is_empty() {
            ...
            println!("✅ Updated snapshot with {} additional fallback blocks", fallback_count);
        }
    }
    
    println!("🎯 Total blocks added: {}, V1 updates: {}", missing_blocks.len(), missing_updates.len());
...
async fn analyze_existing_snapshot_for_all_gaps() -> Result<Vec<SequenceGap>> {
    ...
    println!("🔍 Found {} total gaps across all StateUpdate fields", all_gaps.len());
...
async fn analyze_existing_snapshot() -> Result<Vec<SequenceGap>> {
    ...
    let v1_gaps: Vec<SequenceGap> = all_gaps.into_iter()
        .filter(|gap| {
    ...
    println!("🎯 Found {} gaps specifically in V1 address tree", v1_gaps.len());
...
async fn build_existing_slot_index() -> Result<HashSet<u64>> {
    ...
    let existing_slots: HashSet<u64> = blocks.iter()
...
fn calculate_global_gap_boundaries(gaps: &[SequenceGap]) -> (u64, u64, String, String) {
    ...
    let earliest_before_sig = gaps.iter()
        .min_by_key(|g| g.before_slot)
        .map(|g| g.before_signature.clone())
        .unwrap_or_default();
    
    let latest_after_sig = gaps.iter()
...
async fn fetch_all_signatures_paginated(
    ...
    loop {
        ...
        println!("   📄 Page {}: fetched {} signatures", page_count, batch.len());
        ...
        if reached_until || batch.len() < 1000 { // If we got less than limit, we're done
            break;
        }
        
        // Update before for next page
        current_before = batch.last().and_then(|sig| Signature::from_str(&sig.signature).ok());
    }
    ...
    for (i, slot) in needed_slots.iter().enumerate() {
        match client.get_block_with_config(
            *slot,
                ...
                if let Ok(block_info) = parse_ui_confirmed_blocked(block, *slot) {
                        ...
                        if let Ok(state_update) = parse_transaction(transaction, *slot) {
                            ...
                            if !state_update.indexed_merkle_tree_updates.is_empty() ||
                               ...
                               !state_update.out_accounts.is_empty() {
                                ...
                                println!("   ✅ Found compression activity in slot {} [{}/{}]", slot, i + 1, needed_slots.len());
                                ...
                                println!("   ❌ No compression activity in slot {} [{}/{}]", slot, i + 1, needed_slots.len());
                            }
                        }
                    }
                    ...
                    if has_missing_seq && !slots_with_missing_seqs.contains(slot) {
                        ...
                        println!("   📦 Collected block {} with {} compression transactions [{}/{}]", 
                                 slot, filtered_block.transactions.len(), i + 1, needed_slots.len());
            ...
            Err(e) => {
                println!("   ❌ Failed to fetch slot {} [{}/{}]: {}", slot, i + 1, needed_slots.len(), e);
            }
        }
    }
...
async fn validate_and_fallback_gap_filling(
    ...
    for gap in original_gaps {
        println!("   📊 Checking gap in {:?}: slots {} to {}", 
        ...
        let missing_in_range: Vec<u64> = gap_range_slots.iter()
            .filter(|slot| !existing_slots.contains(slot))
            .copied()
            .collect();
        
        if !missing_in_range.is_empty() {
            println!("   ⚠️  Found {} potentially missing slots in gap range", missing_in_range.len());
    ...
    println!("   🔄 Fallback: fetching {} additional slots from gap ranges", fallback_slots.len());
    println!("   📋 Fallback slots: {:?}", &fallback_slots[..std::cmp::min(10, fallback_slots.len())]);
    
    // Use the same batch fetching approach for fallback slots
    let result = fetch_blocks_batch(client, fallback_slots).await?;
    println!("   ✅ Fallback completed: {} blocks, {} updates", result.0.len(), result.1.len());
    Ok(result)
}
...
async fn fetch_missing_blocks(gaps: &[SequenceGap]) -> Result<(Vec<BlockInfo>, Vec<IndexedTreeLeafUpdate>)> {
    ...
    println!("📊 Found {} existing slots in snapshot", existing_slots.len());
    
    // Phase 1.5: Calculate global gap boundaries
    println!("🌍 Phase 1.5: Calculating global gap boundaries...");
    let (min_slot, max_slot, earliest_before_sig, latest_after_sig) = calculate_global_gap_boundaries(gaps);
    println!("🎯 Global gap range: slots {} to {} (span: {} slots)", 
             min_slot, max_slot, max_slot - min_slot);
    println!("🔗 Global signature range: {} -> {}", 
             &earliest_before_sig[..8], &latest_after_sig[..8]);
    
    // Phase 2: Smart signature collection with pagination
    println!("📡 Phase 2: Fetching ALL signatures with pagination...");
    let all_signatures = fetch_all_signatures_paginated(&client, &earliest_before_sig, &latest_after_sig).await?;
    println!("✅ Collected {} total signatures across all gaps", all_signatures.len());
    
    // Phase 3: Extract and filter slots
    println!("🔍 Phase 3: Extracting and filtering slots...");
    let signature_slots: HashSet<u64> = all_signatures.iter()
        .filter(|sig_info| sig_info.err.is_none()) // Skip failed transactions
        .map(|sig_info| sig_info.slot)
        .collect();
    println!("📊 Found {} unique slots from signatures", signature_slots.len());
    
    // Filter out slots we already have - this is the key optimization!
    let needed_slots: Vec<u64> = signature_slots.iter()
        .filter(|slot| !existing_slots.contains(slot))
        .copied()
        .collect();
    
    println!("🎯 Need to fetch {} new blocks (filtered out {} existing)", 
             needed_slots.len(), signature_slots.len() - needed_slots.len());
    ...
    let (mut missing_blocks, mut missing_updates) = if needed_slots.is_empty() {
        ...
        println!("📦 Phase 4: Fetching {} missing blocks...", needed_slots.len());
        fetch_blocks_batch(&client, needed_slots).await?
    };
    
    println!("🎯 Signature-based approach: found {} blocks, {} updates", missing_blocks.len(), missing_updates.len());
    Ok((missing_blocks, missing_updates))
}
...
async fn verify_gaps_filled() -> Result<()> {
    ...
    for (field_type, field_gaps) in &gaps_by_field {
        ...
        for gap in field_gaps.iter().take(2) { // Show first 2 gaps for each field type
    ...
    println!("ℹ️  Note: Some gaps may remain due to missing blocks on RPC or truly missing sequences");
    Ok(())
}